...
def extract_code_snippet(
    ...
    try:
        ...
        if start_line is None:
            # Return first 30 lines if no specific line is given (e.g. requirements.txt)
            return "".join(lines[:30])

        s_line = max(1, start_line - context)
        e_line = min(len(lines), (end_line or start_line) + context)

        snippet_lines = []
        for line_num in range(s_line, e_line + 1):
            prefix = " > " if start_line <= line_num <= (end_line or start_line) else "   "
            snippet_lines.append(f"{prefix}{line_num:4d} | {lines[line_num - 1]}")
...
@router.get("/api/findings/{finding_id}", response_model=FindingDetailResponse)
...
@router.get("/api/findings/{finding_id}/snippet")
...
def get_scan_findings(
    ...
    scanner: Optional[str] = Query(None, description="Filter by scanner: semgrep, trivy"),
    search: Optional[str] = Query(None, description="Search keyword in title, description, or file path"),
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
